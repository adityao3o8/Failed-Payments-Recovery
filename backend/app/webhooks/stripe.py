...
def _get_or_create_customer(db: Session, stripe_customer_id: str, email: str) -> Customer:
...
def _handle_payment_failed(db: Session, event_data: dict) -> None:
    ...
    customer = _get_or_create_customer(db, customer_id or f"anon_{pi_id}", email)
...
def _handle_invoice_payment_failed(db: Session, event_data: dict) -> None:
    obj = event_data.get("object", {})
